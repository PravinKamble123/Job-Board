...
<template>
    <div class="py-10 px-6 grid md:grid-cols-4 gap-3">
        <div class="md:col-span-3">
            ...
            <a v-bind:href="'mailto:' + job.company_email" class="inline-block mt-6 py-6 px-6 bg-teal-700 text-white rounded-xl">Apply</a>
        </div>
        <div class="md:col-span-1 p-6  bg-teal-700 text-white rounded-xl">
            <h3 class="mb-6 text-xl">Company Details</h3>
            <p class="mb-2">Company Name: {{ job.company_name }}</p>
            <p>Company location: {{ job.company_location }}</p>

            <hr class="my-6 opacity-30">
            <h3 class="mb-6 text-xl">{{ job.title }}</h3>
            <p class="mb-2">Job location: {{ job.position_location }}</p>
